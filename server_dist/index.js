...
// shared/schema.ts
var userRoleEnum = pgEnum("user_role", ["customer", "driver", "admin", "restaurant"]);
var orderStatusEnum = pgEnum("order_status", ["pending", "confirmed", "preparing", "picked_up", "arriving", "delivered", "cancelled"]);
var paymentStatusEnum = pgEnum("payment_status", ["pending", "processing", "completed", "failed", "refunded"]);
var taxStatusEnum = pgEnum("tax_status", ["collected", "remitted", "pending"]);
var remittanceStatusEnum = pgEnum("remittance_status", ["pending", "filed", "paid"]);
var driverStatusEnum = pgEnum("driver_status", ["active", "on_break", "suspended_review", "suspended_safety", "offline"]);
var engagementTierEnum = pgEnum("engagement_tier", ["active", "regular", "casual", "on_break"]);
var interactionTypeEnum = pgEnum("interaction_type", ["wellness_check", "education", "investigation", "appeal"]);
var complianceTypeEnum = pgEnum("compliance_type", ["license", "tax_filing", "agreement", "insurance"]);
var chatTypeEnum = pgEnum("chat_type", ["text", "image", "system"]);
var verificationMethodEnum = pgEnum("verification_method", ["checkout", "delivery"]);
var onrampStatusEnum = pgEnum("onramp_status", ["pending", "processing", "completed", "failed"]);
var offrampStatusEnum = pgEnum("offramp_status", ["pending", "quote_ready", "processing", "completed", "failed"]);
var onboardingStatusEnum = pgEnum("onboarding_status", ["not_started", "in_progress", "pending_review", "approved", "rejected"]);
...
var customers = pgTable2("customers", {
  ...
  idVerificationData: text2("id_verification_data"),
  tastePreferences: json("taste_preferences").$type().default([]),
  dietaryRestrictions: json("dietary_restrictions").$type().default([]),
  referralCode: text2("referral_code"),
  savedAddresses: json("saved_addresses").$type().default([]),
  favoriteRestaurants: json("favorite_restaurants").$type().default([]),
  favoriteItems: json("favorite_items").$type().default([])
}, (table) => [
...
var drivers = pgTable2("drivers", {
  ...
  firstName: text2("first_name").notNull(),
  lastName: text2("last_name").notNull(),
  licenseNumber: text2("license_number"),
  vehicleInfo: text2("vehicle_info"),
  backgroundCheckStatus: text2("background_check_status").default("pending"),
  isAvailable: boolean("is_available").default(false),
  currentLat: real("current_lat"),
  currentLng: real("current_lng"),
  rating: real("rating").default(5),
  totalDeliveries: integer2("total_deliveries").default(0),
  earningsData: json("earnings_data").$type(),
  insuranceData: json("insurance_data").$type(),
...
var driverStatusTable = pgTable2("driver_status_records", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  ...
  suspensionReason: text2("suspension_reason"),
  suspensionDate: timestamp2("suspension_date"),
  reviewStatus: text2("review_status"),
  appealDeadline: timestamp2("appeal_deadline"),
  supportNotes: text2("support_notes"),
  returnDate: timestamp2("return_date"),
  createdAt: timestamp2("created_at").defaultNow().notNull(),
  updatedAt: timestamp2("updated_at").defaultNow().notNull()
});
var driverSupportLog = pgTable2("driver_support_log", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  driverId: varchar("driver_id").notNull().references(() => drivers.id),
  interactionType: interactionTypeEnum("interaction_type").notNull(),
  notes: text2("notes"),
  outcome: text2("outcome"),
  supportRep: text2("support_rep"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var driverEarnings = pgTable2("driver_earnings", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  driverId: varchar("driver_id").notNull().references(() => drivers.id),
  ...
  cryptoCurrency: text2("crypto_currency"),
  cryptoAmount: decimal("crypto_amount", { precision: 18, scale: 8 }),
  usdValue: decimal("usd_value", { precision: 10, scale: 2 }),
  payoutStatus: text2("payout_status").default("pending"),
  paidAt: timestamp2("paid_at"),
  taxYear: integer2("tax_year"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var restaurants = pgTable2("restaurants", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").references(() => users.id),
  name: text2("name").notNull(),
  cuisineType: text2("cuisine_type").notNull(),
  ...
  estimatedPrepTime: text2("estimated_prep_time").default("25-35 min"),
  ...
  distance: text2("distance"),
  isApproved: boolean("is_approved").default(false),
  isSuspended: boolean("is_suspended").default(false),
  agreementSignedDate: timestamp2("agreement_signed_date"),
  agreementData: text2("agreement_data"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var menuItems = pgTable2("menu_items", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  ...
  dietaryTags: json("dietary_tags").$type().default([]),
  available: boolean("available").default(true),
  pairingSuggestions: json("pairing_suggestions").$type().default([]),
  imageUrl: text2("image_url")
}, (table) => [
...
var orders = pgTable2("orders", {
  ...
  paymentProvider: text2("payment_provider").default("stripe"),
  deliveryAddress: text2("delivery_address").notNull(),
...
var bundles = pgTable2("bundles", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  name: text2("name").notNull(),
  items: json("items").$type().notNull(),
  discountPercentage: integer2("discount_percentage").notNull(),
...
var idVerifications = pgTable2("id_verifications", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  customerId: varchar("customer_id").notNull().references(() => customers.id),
  orderId: varchar("order_id").references(() => orders.id),
  scanData: text2("scan_data"),
  verified: boolean("verified").default(false),
  verifiedAt: timestamp2("verified_at"),
  method: verificationMethodEnum("method").notNull(),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var reviews = pgTable2("reviews", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  orderId: varchar("order_id").notNull().references(() => orders.id),
  customerId: varchar("customer_id").notNull().references(() => customers.id),
  restaurantId: varchar("restaurant_id").references(() => restaurants.id),
  driverId: varchar("driver_id").references(() => drivers.id),
  restaurantRating: integer2("restaurant_rating"),
  driverRating: integer2("driver_rating"),
  comment: text2("comment"),
  restaurantResponse: text2("restaurant_response"),
...
var taxJurisdictions = pgTable2("tax_jurisdictions", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  zipCode: text2("zip_code").notNull(),
  city: text2("city").notNull(),
  county: text2("county").notNull(),
...
var taxTransactions = pgTable2("tax_transactions", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  orderId: varchar("order_id").notNull().references(() => orders.id),
...
var taxRemittances = pgTable2("tax_remittances", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  jurisdictionId: varchar("jurisdiction_id").references(() => taxJurisdictions.id),
  ...
  confirmationNumber: text2("confirmation_number"),
  filedDate: timestamp2("filed_date"),
  paidDate: timestamp2("paid_date"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var complianceLogs = pgTable2("compliance_logs", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
...
var deliveryWindows = pgTable2("delivery_windows", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  region: text2("region").notNull(),
  alcoholStartHour: integer2("alcohol_start_hour").default(8),
  alcoholEndHour: integer2("alcohol_end_hour").default(22),
  isActive: boolean("is_active").default(true)
});
var referrals = pgTable2("referrals", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
...
var digitalAgreements = pgTable2("digital_agreements", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  entityType: text2("entity_type").notNull(),
  entityId: varchar("entity_id").notNull(),
  agreementType: text2("agreement_type").notNull(),
  agreementText: text2("agreement_text").notNull(),
  signedAt: timestamp2("signed_at"),
  signatureData: text2("signature_data"),
  ipAddress: text2("ip_address"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var escrowStatusEnum = pgEnum("escrow_status", ["deposited", "released", "disputed", "refunded", "expired"]);
var nftStatusEnum = pgEnum("nft_status", ["pending", "minted", "transferred", "listed", "sold"]);
var wallets = pgTable2("wallets", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  walletAddress: text2("wallet_address").notNull(),
  walletType: text2("wallet_type").default("coinbase"),
  chainId: integer2("chain_id").default(8453),
  isDefault: boolean("is_default").default(true),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var escrowTransactions = pgTable2("escrow_transactions", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  orderId: varchar("order_id").notNull().references(() => orders.id),
  depositorAddress: text2("depositor_address").notNull(),
  sellerAddress: text2("seller_address").notNull(),
  amount: decimal("amount", { precision: 18, scale: 8 }).notNull(),
  tokenAddress: text2("token_address"),
  txHash: text2("tx_hash"),
  escrowId: text2("escrow_id"),
  status: escrowStatusEnum("status").default("deposited"),
  depositedAt: timestamp2("deposited_at").defaultNow().notNull(),
  releasedAt: timestamp2("released_at"),
  disputedAt: timestamp2("disputed_at"),
  refundedAt: timestamp2("refunded_at"),
  chainId: integer2("chain_id").default(8453)
});
var nftCategoryEnum = pgEnum("nft_category", ["milestone", "merchant_dish", "driver_avatar", "customer_loyalty", "marketplace_art"]);
var nftRewards = pgTable2("nft_rewards", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  tokenId: text2("token_id"),
  contractAddress: text2("contract_address"),
  name: text2("name").notNull(),
  description: text2("description"),
  imageUrl: text2("image_url"),
  metadataUri: text2("metadata_uri"),
  milestoneType: text2("milestone_type").notNull(),
  milestoneValue: integer2("milestone_value"),
  txHash: text2("tx_hash"),
  status: nftStatusEnum("status").default("pending"),
  mintedAt: timestamp2("minted_at"),
  listedPrice: decimal("listed_price", { precision: 18, scale: 8 }),
  chainId: integer2("chain_id").default(8453),
  nftCategory: nftCategoryEnum("nft_category").default("milestone"),
  aiGenerated: boolean("ai_generated").default(false),
  aiPrompt: text2("ai_prompt"),
  restaurantId: varchar("restaurant_id"),
  dishName: text2("dish_name"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var nftListings = pgTable2("nft_listings", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  ...
  currency: text2("currency").default("USDC"),
  status: text2("listing_status").default("active"),
  txHash: text2("tx_hash"),
  listedAt: timestamp2("listed_at").defaultNow().notNull(),
  soldAt: timestamp2("sold_at")
});
var onrampTransactions = pgTable2("onramp_transactions", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  walletAddress: text2("wallet_address").notNull(),
  fiatCurrency: text2("fiat_currency").default("USD"),
  fiatAmount: decimal("fiat_amount", { precision: 18, scale: 2 }).notNull(),
  cryptoCurrency: text2("crypto_currency").default("USDC"),
  cryptoAmount: decimal("crypto_amount", { precision: 18, scale: 8 }),
  network: text2("network").default("base"),
  paymentMethod: text2("payment_method"),
  coinbaseTransactionId: text2("coinbase_transaction_id"),
  status: onrampStatusEnum("status").default("pending"),
  completedAt: timestamp2("completed_at"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var offrampTransactions = pgTable2("offramp_transactions", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  walletAddress: text2("wallet_address").notNull(),
  cryptoCurrency: text2("crypto_currency").default("USDC"),
  ...
  network: text2("network").default("base"),
  cashoutMethod: text2("cashout_method").default("BANK_ACCOUNT"),
  coinbaseTransactionId: text2("coinbase_transaction_id"),
  quoteId: text2("quote_id"),
  ...
  estimatedArrival: text2("estimated_arrival"),
  completedAt: timestamp2("completed_at"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var pushTokens = pgTable2("push_tokens", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  token: text2("token").notNull(),
  platform: text2("platform").default("expo"),
  active: boolean("active").default(true),
  createdAt: timestamp2("created_at").defaultNow().notNull(),
  updatedAt: timestamp2("updated_at").defaultNow().notNull()
});
var onboardingApplications = pgTable2("onboarding_applications", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  role: text2("role").notNull(),
  status: onboardingStatusEnum("status").notNull().default("not_started"),
  step: integer2("step").default(1),
  businessName: text2("business_name"),
  businessAddress: text2("business_address"),
  businessPhone: text2("business_phone"),
  einNumber: text2("ein_number"),
  cuisineType: text2("cuisine_type"),
  hasAlcoholLicense: boolean("has_alcohol_license").default(false),
  alcoholLicenseNumber: text2("alcohol_license_number"),
  operatingHoursData: json("operating_hours_data").$type(),
  licenseNumber: text2("license_number"),
  vehicleType: text2("vehicle_type"),
  vehicleMake: text2("vehicle_make"),
  vehicleModel: text2("vehicle_model"),
  vehicleYear: text2("vehicle_year"),
  vehicleColor: text2("vehicle_color"),
  licensePlate: text2("license_plate"),
  insuranceProvider: text2("insurance_provider"),
  insurancePolicyNumber: text2("insurance_policy_number"),
  ...
  agreementSignedAt: timestamp2("agreement_signed_at"),
...
var apiKeyTierEnum = pgEnum("api_key_tier", ["free", "starter", "pro", "enterprise"]);
var apiKeys = pgTable2("api_keys", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  name: text2("name").notNull(),
  publicKey: text2("public_key").notNull().unique(),
  secretKeyHash: text2("secret_key_hash").notNull(),
  tier: apiKeyTierEnum("tier").notNull().default("free"),
  isActive: boolean("is_active").default(true),
  isSandbox: boolean("is_sandbox").default(true),
  rateLimit: integer2("rate_limit").default(100),
  dailyRequests: integer2("daily_requests").default(0),
  lastResetAt: timestamp2("last_reset_at").defaultNow(),
  permissions: json("permissions").$type().default(["read"]),
  allowedOrigins: json("allowed_origins").$type().default([]),
...
var webhookEventEnum = pgEnum("webhook_event", [
  "order.created",
  "order.confirmed",
  "order.preparing",
  "order.picked_up",
...
var webhooks = pgTable2("webhooks", {
  ...
  isActive: boolean("is_active").default(true),
  failureCount: integer2("failure_count").default(0),
  lastDeliveredAt: timestamp2("last_delivered_at"),
  createdAt: timestamp2("created_at").defaultNow().notNull(),
  updatedAt: timestamp2("updated_at").defaultNow().notNull()
});
var webhookDeliveries = pgTable2("webhook_deliveries", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  ...
  attempts: integer2("attempts").default(1),
  success: boolean("success").default(false),
  deliveredAt: timestamp2("delivered_at").defaultNow().notNull()
});
var integrationPartners = pgTable2("integration_partners", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  apiKeyId: varchar("api_key_id").notNull().references(() => apiKeys.id),
  name: text2("name").notNull(),
...
var whiteLabelConfigs = pgTable2("white_label_configs", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  apiKeyId: varchar("api_key_id").notNull().references(() => apiKeys.id),
  brandName: text2("brand_name").notNull(),
  primaryColor: text2("primary_color").default("#FF6B00"),
  secondaryColor: text2("secondary_color").default("#1A1A2E"),
  accentColor: text2("accent_color").default("#00D4AA"),
  logoUrl: text2("logo_url"),
  faviconUrl: text2("favicon_url"),
  customDomain: text2("custom_domain"),
  customCss: text2("custom_css"),
  footerText: text2("footer_text"),
  supportEmail: text2("support_email"),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp2("created_at").defaultNow().notNull(),
  updatedAt: timestamp2("updated_at").defaultNow().notNull()
});
var apiAuditLogs = pgTable2("api_audit_logs", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  ...
  requestBody: json("request_body").$type(),
  responseTime: integer2("response_time"),
  ipAddress: text2("ip_address"),
  userAgent: text2("user_agent"),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
var inboundOrders = pgTable2("inbound_orders", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  apiKeyId: varchar("api_key_id").notNull().references(() => apiKeys.id),
  externalOrderId: text2("external_order_id").notNull(),
  ...
  total: decimal("total", { precision: 10, scale: 2 }).notNull(),
  specialInstructions: text2("special_instructions"),
  status: text2("status").default("received"),
  internalOrderId: varchar("internal_order_id"),
  metadata: json("metadata").$type(),
  createdAt: timestamp2("created_at").defaultNow().notNull(),
  updatedAt: timestamp2("updated_at").defaultNow().notNull()
});
var agreementTypeEnum = pgEnum("agreement_type", [
  "terms_of_service",
  "privacy_policy",
...
var legalAgreements = pgTable2("legal_agreements", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id),
  agreementType: agreementTypeEnum("agreement_type").notNull(),
  version: text2("version").notNull().default("1.0"),
  acceptedAt: timestamp2("accepted_at").defaultNow().notNull(),
  ipAddress: text2("ip_address"),
  userAgent: text2("user_agent"),
  documentHash: text2("document_hash"),
  metadata: json("metadata").$type()
});
var licenseVerifications = pgTable2("license_verifications", {
  id: varchar("id").primaryKey().default(sql2`gen_random_uuid()`),
  restaurantId: varchar("restaurant_id"),
  onboardingId: varchar("onboarding_id"),
  licenseNumber: text2("license_number").notNull(),
  businessName: text2("business_name"),
  verificationMethod: text2("verification_method").notNull(),
  status: text2("status").notNull().default("pending"),
  licenseType: text2("license_type"),
  expirationDate: text2("expiration_date"),
  county: text2("county"),
  details: json("details").$type(),
  verifiedAt: timestamp2("verified_at").defaultNow().notNull(),
  createdAt: timestamp2("created_at").defaultNow().notNull()
});
...
var merchantOnboardingSchema = z.object({
...
var driverOnboardingSchema = z.object({
...
var storage = {
  ...
  async updateDriverStatus(driverId, data) {
    ...
    if (existing) {
      const [status2] = await db.update(driverStatusTable).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(driverStatusTable.id, existing.id)).returning();
      return status2;
    }
  ...
  async createDriverSupportLog(data) {
    const [log2] = await db.insert(driverSupportLog).values(data).returning();
    return log2;
  },
  ...
  async createComplianceLog(data) {
    const [log2] = await db.insert(complianceLogs).values(data).returning();
    return log2;
  },
  ...
  async savePushToken(userId, token, platform) {
    ...
    if (existing.length > 0) {
      const [updated] = await db.update(pushTokens).set({ active: true, updatedAt: /* @__PURE__ */ new Date() }).where(eq(pushTokens.id, existing[0].id)).returning();
      return updated;
    }
  ...
  async getOnboardingByUserId(userId) {
    const [app2] = await db.select().from(onboardingApplications).where(eq(onboardingApplications.userId, userId)).limit(1);
    return app2;
  },
  async getOnboardingById(id) {
    const [app2] = await db.select().from(onboardingApplications).where(eq(onboardingApplications.id, id)).limit(1);
    return app2;
  },
  async updateOnboarding(id, data) {
    const [app2] = await db.update(onboardingApplications).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(onboardingApplications.id, id)).returning();
    return app2;
  },
  async getPendingOnboardings() {
    return db.select().from(onboardingApplications).where(eq(onboardingApplications.status, "pending_review")).orderBy(desc(onboardingApplications.createdAt));
  },
  async getAllOnboardings() {
    return db.select().from(onboardingApplications).orderBy(desc(onboardingApplications.createdAt));
  },
  ...
  async getLegalAgreementsByUser(userId) {
    return db.select().from(legalAgreements).where(eq(legalAgreements.userId, userId)).orderBy(desc(legalAgreements.acceptedAt));
  },
  async hasAcceptedAgreement(userId, agreementType, version) {
    const conditions = [
      eq(legalAgreements.userId, userId),
      eq(legalAgreements.agreementType, agreementType)
    ];
  ...
  async getLicenseVerificationsByRestaurant(restaurantId) {
    return db.select().from(licenseVerifications).where(eq(licenseVerifications.restaurantId, restaurantId)).orderBy(desc(licenseVerifications.createdAt));
  },
...
async function registerRoutes(app2) {
  ...
  app2.post("/api/driver/support", authMiddleware, async (req, res) => {
    try {
      const driver = await storage.getDriverByUserId(req.user.id);
      if (!driver) return res.status(404).json({ message: "Driver profile not found" });
      const log2 = await storage.createDriverSupportLog({
        driverId: driver.id,
...
function generatePublicKey() {
  return `ce_pk_${crypto4.randomBytes(24).toString("hex")}`;
}
function generateSecretKey() {
  return `ce_sk_${crypto4.randomBytes(32).toString("hex")}`;
}
function generateWebhookSecret() {
  return `whsec_${crypto4.randomBytes(32).toString("hex")}`;
}
...
var platformStorage = {
  ...
  async createAuditLog(data) {
    const [log2] = await db.insert(apiAuditLogs).values(data).returning();
    return log2;
  },
...
var createApiKeySchema = z2.object({
  ...
  tier: z2.enum(["free", "starter", "pro", "enterprise"]).default("free"),
  isSandbox: z2.boolean().default(true)
});
var createWebhookSchema = z2.object({
  url: z2.string().url(),
  events: z2.array(z2.string()).min(1)
});
var inboundOrderSchema = z2.object({
  externalOrderId: z2.string().min(1),
  ...
  items: z2.array(z2.object({
    ...
    isAlcohol: z2.boolean().optional()
  })).min(1),
  subtotal: z2.number().positive(),
  deliveryFee: z2.number().min(0).optional(),
  tip: z2.number().min(0).optional(),
  total: z2.number().positive(),
  specialInstructions: z2.string().optional(),
  metadata: z2.record(z2.unknown()).optional()
});
var whiteLabelSchema = z2.object({
  brandName: z2.string().min(1),
  ...
  customDomain: z2.string().optional(),
  supportEmail: z2.string().email().optional()
});
function registerPlatformRoutes(app2) {
  const platformLimiter = rateLimit2({
    windowMs: 60 * 1e3,
    max: 60,
...
(async () => {
  ...
  app.get("/legal/tos", (_req, res) => {
    const tosPath = path4.resolve(process.cwd(), "server", "templates", "terms-of-service.html");
  ...
  app.get("/legal/contractor", (_req, res) => {
    const contractorPath = path4.resolve(process.cwd(), "server", "templates", "contractor-agreement.html");
